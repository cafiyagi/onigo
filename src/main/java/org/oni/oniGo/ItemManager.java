...
public class ItemManager {
    ...
    // 追加: 転生ポイント記録
    private Map<UUID, Location> tennseiLocations = new HashMap<>();
    ...
    public void giveAllGameItems(Player player) {
        ...
        // 転生アイテム追加
        player.getInventory().addItem(createTennseiItem());
    ...
    public void distributeTeamItems() {
        for (Player p : plugin.getServer().getOnlinePlayers()) {
            ...
            if (teamManager.isPlayerInOniTeam(p)) {
                ...
                p.getInventory().addItem(createTennseiItem()); // 転生アイテム追加
    ...
    // 転生ポイント記録/利用
    public void handleTennseiItem(Player player) {
        UUID playerId = player.getUniqueId();

        // 既に記録されているか確認
        if (tennseiLocations.containsKey(playerId)) {
            // 記録位置にテレポート
            Location savedLocation = tennseiLocations.get(playerId);
            player.teleport(savedLocation);
            player.sendMessage(ChatColor.LIGHT_PURPLE + "転生地点にテレポートしました");

            // 使用後は記録を削除
            tennseiLocations.remove(playerId);
        } else {
            // 現在位置を記録
            tennseiLocations.put(playerId, player.getLocation().clone());
            player.sendMessage(ChatColor.LIGHT_PURPLE + "転生地点を記録しました。再度使用するとこの場所に戻れます");
        }
    }
    ...
    // 転生アイテム判定
    public boolean isTennseiItem(ItemStack item) {
        return item != null &&
    ...
    public void resetAllCooldowns() {
        ...
        tennseiLocations.clear(); // 転生ポイントもリセット
